...
import { bindGroupManager, windowManager, cameraManager, renderTargetManager, layerManager, textureManager, meshManager, modelManager, renderShaderManager, entityManager, bufferManager, skeletonManager, computeShaderManager } from "../../managers.js";
...
export const meshEditorInit = async () => {
    ...
    var uvCamera = cameraManager.addCamera("uvCamera", [0, 0, -1], 90, 16 / 9, 0.01);
    uvCamera.rotation.lookAt(uvCamera.position, vec3.zero, vec3.up);
    ...
    var mainWindow = windowManager.addWindow("mainWindow", [0.001, 0.042], 0.807, 0.957, 0, "mainCamera");
    ...
    uvLinesBuffer.addToBuffer(xLine);
    uvLinesBuffer.addToBuffer(yLine);
    uvLinesBuffer.write();
    ...
    bufferManager.addBuffer("meshVertexBuffer", 6, maxMeshVertices, { type: "float32", usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
    bufferManager.addBuffer("meshIndexBuffer", 3, maxMeshVertices, { type: "uint32", usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST });
    bufferManager.addBuffer("selectedTrianglesVertexBuffer", 6, maxMeshVertices * 3, { type: "float32", usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
    bufferManager.addBuffer("tempTrianglesVertexBuffer", 3, maxMeshVertices * 3, { type: "float32", usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
...
export const meshEditorUpdate = (dt) => {
    updateUIElements();
