...
window.onmousemove = e => {
    ...
    user.hoveredHTMLElement =  (e.target as HTMLElement)
}
...
var windowsToCheck: Array<[string, number]> = []
var rayPos = new vec3()
var rayDir = new vec3()
...
export const user: {
    ...
    hoveredWindows: Set<string>,
    hoveredHTMLElement: HTMLElement,
    updateEvents: () => void,
    checkEvents: (includedEvents: string[], excludedEvents?: string[]) => boolean,
    resetEvents: () => void,
    updateHoveredEntity: () => void
} = {
    eventCodes: {},
    events: new Set(),
            ...
            for (var i = window.layers.length - 1; i >= 0; i--) {
                ...
                for (var entityID of layer.entities) {
                    ...
                    for (var j = 0; j < entity.meshes.length; j++) {
                        ...
                        if (entityMesh.visibleMesh == undefined) {continue}
