...
export const load = async () => {
    ...
    const mesh: meshInfo = JSON.parse(await window.fs.readGlobalFile(filePath, "utf8"))
...
export type templates = "cube" | "quad"
export const loadTemplate = async (template: templates) => {
    var templateToLoad = ""
    switch (template) {
        case "cube":
    ...
    const mesh: meshInfo = JSON.parse(await window.fs.readFile(templateToLoad, "utf8"))
    if (mesh.vertices == undefined || mesh.texcoords == undefined || mesh.normals == undefined || mesh.triangles == undefined) {
        console.log("Invalid mesh info")
        return
    }
