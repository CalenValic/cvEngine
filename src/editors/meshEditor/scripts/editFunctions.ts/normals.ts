...
export const flipNormals = () => {
    for (var triangleID of meshData.selectedTriangles) {
        ...
        meshData.triangles[triangleID].texcoords.reverse()
        meshData.triangles[triangleID].texEdges.reverse()
    }
}
...
var edgeIntersections: Set<string> = new Set()
const calculateTriangleNormalIntersections = (triangleID: string): number => {
...
var trianglesToFlip: Set<string> = new Set()
...
var normal = new vec3()
var normalsIDMap: Map<string, string> = new Map()
export const recalculateVertexNormals = () => {
    ...
    for (var vertexID in meshData.vertices) {
        ...
        if (!normalsIDMap.has(normalString)) {
            var normalID = crypto.randomUUID() as string
