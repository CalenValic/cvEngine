...
export const updateUIElements = () => {
    if (user.checkEvents(["RightMouseDown"]) && meshData.currentlyEditing == "mesh") {
    ...
    if (user.checkEvents(["RightMouseDown"]) && meshData.currentlyEditing == "uvs") {
        if (user.hoveredWindows.has("mainWindow") || user.hoveredWindows.has("uvWindow")) {
            UI.toggleClasses("uvRightClickMenu", ["showFlex"], "add")
            UI.updateStyles("uvRightClickMenu", [
                ["top", user.mousePosition.y * 100 + 0.1 + "%"],
                ["left", user.mousePosition.x * 100 + 0.1 + "%"]
            ])
        } else {
    ...
    if (user.checkEvents(["EscapeDown"])) {
        UI.toggleClasses("fileButton", ["showDropdown"], "remove")
        UI.toggleClasses("showButton", ["showDropdown"], "remove")
        UI.toggleClasses("editButton", ["showDropdown"], "remove")
    }
}